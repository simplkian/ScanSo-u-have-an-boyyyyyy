...
|---------|-------|
| **Runtime** | Node |
| **Build Command** | `npm install && npm run server:build && npm run expo:static:build` |
...
3. Render will detect and configure everything automatically

### Post-Deployment
1. Your API will be available at `https://your-app-name.onrender.com`
2. Update `EXPO_PUBLIC_DOMAIN` in your Expo app to point to the Render URL
3. The mobile app connects to the Render backend via Expo Go
